...
SUPABASE_KEY = "sb_publishable_DVKxkmdsCdGS30c_ur02Dg_sljzJ-fg"
# ==========================================
...
<style>
    div[data-testid="stChatMessage"]:has(div[data-testid="stChatMessageAvatarAssistant"]) {
        background-color: transparent !important; border-left: 3px solid #888 !important; 
        border-radius: 0 !important; padding-left: 15px !important; margin-top: 20px !important;
        margin-bottom: 20px !important; width: fit-content !important; max-width: 85% !important;
    }
    div[data-testid="stChatMessage"]:has(div[data-testid="stChatMessageAvatarAssistant"]) p {
        font-size: 15px !important; color: #333 !important; line-height: 1.6 !important;
    }
    div[data-testid="stChatMessage"]:has(div[data-testid="stChatMessageAvatarUser"]) {
        background-color: #f0f2f6 !important; border-radius: 16px !important;
        padding: 12px 18px !important; margin-top: 15px !important;
        margin-bottom: 15px !important; margin-left: auto !important;
    ...
    if st.button("提交"):
        try:
            if auth_mode == "注册":
                ...
                st.success("注册成功！请直接登录（Supabase默认开启了邮箱验证，你可以在后台关闭它，或者去邮箱点确认链接）。")
            else:
...
with st.sidebar:
    st.header("⚙️ 设定与接入")
    # (为节省篇幅，API配置和头像设置代码保留你原来的写法，此处略过，如果需要完整版请告诉我)
